...
impl Keybindings {
    pub fn new() -> Self {
        let all = vec![
            ...
            kb!(vec![Char('i')], Action::ToggleInfo, Global),
            kb!(vec![Char('F')], Action::ToggleFavorite, Panel(Panel::TrackList)),
            ...
            kb!(vec![Tab], Action::OverlayNavigate(1), Overlay),
        ];
    ...
    pub fn help_sections(&self) -> Vec<(&'static str, Vec<(String, &'static str)>)> {
        ...
        vec![
            ...
            ("Library", vec![
                ...
                (k(ToggleInfo), "Show detailed info & album art for selected track"),
                (k(ToggleFavorite), "Toggle favorite on selected track(s)"),
            ]),
            ("Playlists", vec![
